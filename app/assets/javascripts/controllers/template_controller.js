...
Controllers.controller('templateCtrl', ['$scope', 'Template', '$routeParams', '$window', '$http',
    function($scope, Template, $routeParams, $window, $http) {

        $scope.templates = Template.index();

        $scope.curPage = 0;
        $scope.pageSize = 3;
        $scope.templates.$promise.then(function(data){
        ...
        $http.get("templates/all_questions").then(function(response) {
            $scope.all_question = response.data;
        });
        $http.get('templates/questions').then(function(response) {
        ...
        $scope.addItem = function () {
            q_id = parseInt($scope.selectedQuestion.substring(0, parseInt($scope.selectedQuestion.indexOf('|'))));
            $scope.errortext = "";
            if (!$scope.selectedQuestion) {return;}
            if ($scope.questions.indexOf($scope.selectedQuestion) == -1) {
                $scope.questions.push($scope.selectedQuestion);
                question_ids.push(q_id);
            } else {
                $scope.errortext = " The item is already in your question list";
            }
        };
        ...
        $scope.addTemplate = function() {
            $scope.newTemplate.question_ids = question_ids;
            var template = Template.save($scope.newTemplate).$promise.then(function(success) {
...
Controllers.controller('templateShowCtrl', ['$scope', '$routeParams', 'Template',
    function($scope, $routeParams, Template) {
        $scope.template = Template.get({id: $routeParams.id});
    }]);

Controllers.controller('templateEditCtrl', ['$scope', '$routeParams', 'Template', '$window', '$http', '$filter',
    function ($scope, $routeParams, Template, $window, $http, $filter) {
        var question_ids = [];
        $scope.template = Template.get({id: $routeParams.id});
        ...
        $scope.addItem = function () {
            //var q_id = parseInt($scope.selectedQuestion.substring(0, parseInt($scope.selectedQuestion.indexOf('|'))));
            q_id = ($filter('getByContent')($scope.all_question, $scope.selectedQuestion)).id;
            question_ids.push(q_id);
            $scope.errortext = "";
            if (!$scope.selectedQuestion) {return;}
            if ($scope.template.questions.indexOf($scope.selectedQuestion) == -1) {
                $scope.template.questions.push($scope.selectedQuestion);
        ...
        $scope.editTemplate = function(template){
            var content_of_template, question_of_template, q_id;
            $scope.template.$promise.then(function(data) {
                $scope.template = data;
                content_of_template = $scope.template.questions;
                var i, len = content_of_template.length;
                for (i=0; i<len; i++) {
                    question_of_template = $filter('getByContent')($scope.all_question, content_of_template[i]);
                    question_ids.push(question_of_template.id)
                }
            });
            $scope.template.question_ids = question_ids;
            Template.update({id: $routeParams.id, name: $scope.template.name, question_ids: question_ids}).$promise.then(function() {
                $window.location.href = 'templates#';
            });
        };

        $scope.removeItem = function (x) {
            $scope.template.questions.splice(x, 1);
        };
    }]);

//Controllers.controller('ctrlRead', function ($scope, $filter, Template, $http, $routeParams, $window) {
//
//    // init
//    $scope.sort = {
//        sortingOrder : 'id',
...
//
//
//Controllers.$inject = ['$scope', '$filter'];
//
//Controllers.directive("customSort", function() {
//    return {
//        restrict: 'A',
//        transclude: true,
//        scope: {
